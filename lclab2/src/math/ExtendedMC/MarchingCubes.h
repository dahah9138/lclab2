...
namespace LC { namespace ExtendedMC {
	...
	/** \struct Vertex "MarchingCubes.h" MarchingCubes
		...
		*/
		...
		// Lis Modification
		// I need to change the struct name because of the another struct with the same name on Edge_Flow
	typedef struct
	{
	...
	public:
		...
		void clean_all();
		/** clears all isosurface structures : the mesh buffers */
		void clean_mesh();
		/** resets mesh index counters */
		void reset_mesh();
		//-----------------------------------------------------------------------------
		// Exportation
	public:
