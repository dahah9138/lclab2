...
class Simulation {
	...
	struct ParameterList {
		...
		// Shear modulus (Pa)
		LC::scalar mu = 570e3;
		// Coupling coefficient (0.175 mu)
		LC::scalar alpha = .175 * 570e3;
...
public:
	...
	*/
	void InitVerticesAndElements(const LC::scalar* directors = 0,
		const std::array<int, 3>& voxels = { 10,10,5 },
		const std::array<LC::scalar, 3>& cell_dims = {10.,10.,5.});
...
void Simulation::InitVerticesAndElements(const LC::scalar* directors,
	...
	if (directors) {
		LC_WARN("Cannot specify dynamic director field configuration for vertices yet!");
	}
	else {
			...
			for (int j = 0; j < voxels[1]; j++) {
				for (int k = 0; k < voxels[2]; k++) {
					...
					for (int d = 0; d < 3; d++) {
						...
						m_nodes[idx].position[d] = -0.5 * voxels[d] + ri[d];
					...
					// Initialize the vertex indices defining the element
					if (i < evoxels[0] && j < evoxels[1] && k < evoxels[2]) {
						for (int a = 0; a < 2; a++)
							for (int b = 0; b < 2; b++)
								for (int c = 0; c < 2; c++)
									m_elements[elem_idx][a + 2 * b + 4 * c] = index(i + a, j + b, k + c);
					}
				}
			}
		}
...
void Simulation::updateDynamicsAtElement(const unsigned int& elemIdx) {
	...
	*/
	...
	for (int i = 0; i < 8; i++) {
		...
		// Scale node positions from um to m
		if (PBC) {
			// Translate to the opposite side of the cell if on one edge
			for (int d = 0; d < 2; d++) {
				LC::scalar dr = m_nodes[m_elements[elemIdx][i]].position[d] - 0.5 * m_params.cell_dims[d];
				...
				r_i[i][d] = (m_nodes[m_elements[elemIdx][i]].position[d] + dr);
			}
			// No PBCs in z-direction
			r_i[i][2] = m_nodes[m_elements[elemIdx][i]].position[2];
		}
		else
			r_i[i] = m_nodes[m_elements[elemIdx][i]].position;
	}
	...
	for (int d = 0; d < 3; d++) {
		ra[d] = 1. / 8. * (-r_i[0][d] + r_i[1][d] - r_i[2][d] + r_i[3][d] - r_i[4][d] + r_i[5][d] - r_i[6][d] + r_i[7][d]);
		rb[d] = 1. / 8. * (-r_i[0][d] - r_i[1][d] + r_i[2][d] + r_i[3][d] - r_i[4][d] - r_i[5][d] + r_i[6][d] + r_i[7][d]);
		rg[d] = 1. / 8. * (-r_i[0][d] - r_i[1][d] - r_i[2][d] - r_i[3][d] + r_i[4][d] + r_i[5][d] + r_i[6][d] + r_i[7][d]);
		rab[d] = 1. / 8. * (r_i[0][d] - r_i[1][d] - r_i[2][d] + r_i[3][d] + r_i[4][d] - r_i[5][d] - r_i[6][d] + r_i[7][d]);
		rag[d] = 1. / 8. * (r_i[0][d] - r_i[1][d] + r_i[2][d] - r_i[3][d] - r_i[4][d] + r_i[5][d] - r_i[6][d] + r_i[7][d]);
		rbg[d] = 1. / 8. * (r_i[0][d] + r_i[1][d] - r_i[2][d] - r_i[3][d] - r_i[4][d] - r_i[5][d] + r_i[6][d] + r_i[7][d]);
		rabg[d] = 1. / 8. * (-r_i[0][d] + r_i[1][d] + r_i[2][d] - r_i[3][d] + r_i[4][d] - r_i[5][d] - r_i[6][d] + r_i[7][d]);
	}
	...
	Eigen::Vector3d _4gradI, _8gradJ, _4gradQeps;
	// Iterate over the eight grid points (+-gInt, +-gInt, +-gInt) computing the force

	LC::scalar avglen = 0.0;
	for (int d = 0; d < 3; d++)
		avglen += 1./3. * 1e-6 * m_params.cell_dims[d] / (m_params.voxels[d] - 1);
	...
	for (int i = -1; i <= 1; i += 2) {
		for (int j = -1; j <= 1; j += 2) {
			for (int k = -1; k <= 1; k += 2) {
				...
				// Do not update if the volume elem is negative
				if (J < 0.)
					continue;
				...
				Eigen::Vector3d n = m_nodes[m_elements[elemIdx][idx]].director;
				...
				_4gradI = bg * drda_eval + ag * drdb_eval + ab * drdg_eval;
				_8gradJ = bg * drdb_eval.cross(drdg_eval) + ag * drdg_eval.cross(drda_eval) + ab * drda_eval.cross(drdb_eval);
				_4gradQeps = (n[0] * bg + n[1] * ag + n[2] * ab) * (n[0] * drda_eval + n[1] * drdb_eval + n[2] * drdg_eval);
				...
				Force term3 = UdS * (1. / J2_3 * 0.25 * _4gradQeps - 0.25 * _4gradI / 3. / J2_3 - 2. * (n_an_beps_ab - I1 / 3.) * _8gradJ / 8. / 3. / pow(J, 5. / 3.));

				//LC_INFO("log(J)/J = {0}", log(J) / J);
				//LC_INFO("I_1 = {0}", I1);

				force = -0.5 * m_params.mu * (I_m - 3) / (I_m - I1) * 0.25 * _4gradI
					- m_params.kappa * log(J) / J * _8gradJ
					+ term3;
				...
				m_nodes[m_elements[elemIdx][idx]].force += avglen * avglen * force;
				++idx;			
			}
		}
	}
...
void Simulation::updateNodes() {
	...
	// Compute acceleration/velocity contributions over all nodes
	...
	for (unsigned int i = 0; i < m_nNodes; i++) {
		...
		LC::scalar eta = 0.1;
		Eigen::Vector3d accel = 1. / m_nodes[i].mass * m_nodes[i].force;
		...
		// Don't allow node to pass through glass
		if (m_nodes[i].position[2] < -0.5 * m_params.voxels[2])
			m_nodes[i].position[2] = -0.5 * m_params.voxels[2];
